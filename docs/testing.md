...
- [`ci.yml`](../.github/workflows/ci.yml) — runs on every PR and push to `main`. Type checks, Studio checks, glossary source and browser contracts, Portable Text headings, drop lookup, psychrometrics, and navigation, then calls the audit. Pushes also check nightly freshness.
- [`audit.yml`](../.github/workflows/audit.yml) — **reusable** workflow (`workflow_call`) that builds once, then validates sitemap XML, robots.txt, analytics, content style, heading anchors, glossary output, links, and Lighthouse. Called by both CI and the nightly job.
...
- [`deploy.yml`](../.github/workflows/deploy.yml) — builds and publishes to Pages. Not an audit workflow, but it carries the two content-sensitive blocking checks, and it is the **only** workflow on the path a content publish takes. See [Content publishes skip the audit workflow](#content-publishes-skip-the-audit-workflow).
...
- **Validate glossary reference-library output** (local aggregate, CI audit, and deploy) — `make check-glossary-build` derives the expected directory from every built glossary detail page instead of pinning a document count or a permanently concise example. For every directory entry it verifies the controller hooks, canonical term, initial, category, normalized search text, visible definition and category, and link. Featured cards must match their directory text and detail image; constrained card and specimen URLs must carry Sanity crop rectangles, and optional-body pages must render reading time and contents only when their content requires them. `make check` also runs isolated malformed-fixture regressions for these contracts. The deploy copy fails before GitHub Pages receives the artifact, leaving the prior deployment live.
...
The automated glossary check confirms that each constrained URL carries a Sanity `rect` parameter and that the featured card and detail page use the same image asset and alt text. Only this manual check can confirm that the chosen rectangle preserves the intended subject.
...
- **Content metadata outside the glossary contract** (ghost terpenes and missing fields on other document types) — covered by the [`/audit-content`](../README.md#claude-code-skills) skill, which can be run on a schedule rather than as a CI gate. Glossary image alt text and featured-entry requirements are blocking build contracts.
