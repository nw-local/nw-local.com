...
- [`deploy.yml`](../.github/workflows/deploy.yml) — builds and publishes to Pages. Not an audit workflow, but it carries the four content-sensitive blocking checks, and it is the **only** workflow on the path a content publish takes. See [Content publishes skip the audit workflow](#content-publishes-skip-the-audit-workflow).
...
- **Validate drop build contracts** (local aggregate, CI, and deploy) — `make check-drop-build` (`scripts/check-drop-build.py`) verifies every built drop page against its own certificate manifest: the page's rendered COA links must equal `dist/drops/<slug>/coas.json`, every linked certificate page must exist, a declared `strain.lineage` must be visible on the page, and every image must be served from `cdn.sanity.io`. `make test-check-drop-build` (`scripts/test-check-drop-build.py`) runs the same checker against malformed fixtures to prove it fails closed, and `make test-drops` runs `drops.ts`'s dependency-free validation/grouping tests plus a Container API render of one chapter. The deploy copy protects `drop.coas`, `strain.lineage`, and `drop.gallery`, all of which can change through Sanity with no pull request.
...
`deploy.yml` therefore runs `check-content-style.py`, `check-email-routing.py`, `check-glossary-build.py`, and `check-drop-build.py` against the `dist/` the build step just produced. They are the final validation steps in the `build` job and `deploy` declares `needs: build`, so a failure in any of them skips deployment entirely and the previously deployed site stays live. **The visible site symptom of a rejected publish is that the content does not appear.** If an editor publishes and the change never shows up, the failed deploy run is the first place to look.

Four checks are duplicated here: content style, email routing, the glossary build contract, and the drop build contract. `robots` and the analytics snippet can only be broken by a code change, which always goes through a pull request and is already covered. Link checking is another check content can genuinely break — a post linking to a deleted page — but it needs the built site served and is much slower, and `deploy.yml` is already serialized behind a Pages concurrency group that a document-batch publish floods. The nightly run covers it instead, within 24 hours.
...
The drop build contract is duplicated for the same reason: `drop.coas`, `strain.lineage`, and `drop.gallery` can all change through a Sanity publish with no code diff, and `check-drop-build.py` reads the emitted drop pages against their own `coas.json` manifest to confirm the two paths still agree.
