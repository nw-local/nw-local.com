...
An author's optional direct email is similarly person-scoped: it renders only on that author's profile and is emitted on the profile's `Person` JSON-LD. It does not enter the compact author projection used by blog cards and article bylines. The organization's general address remains `siteSettings.contactEmail`, which drives the footer, contact page, and `Organization` JSON-LD independently.
