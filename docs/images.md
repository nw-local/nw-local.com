...
**A hotspot only survives if the GROQ projection selects it.** `urlFor()` reads the object it is handed, so `image { asset->, alt }` produces a center crop no matter what the editor set in the Studio, while `image { asset->, alt, crop, hotspot }` produces the framing they chose. The Studio previews the reframed crop either way, because it reads the schema rather than the query, so the mismatch shows up only on the built page. Nothing flags it: `SanityImage` declares both fields optional, so the narrower projection type-checks clean, and the image still renders, just framed wrong. Both `product.image` projections had this bug until the first product documents were added, and their square jar photos each lost the strain name off the bottom of the label. To check a projection rather than trust it, read the `rect=` parameter on the generated URL: it differs between a center crop and an honored hotspot on the same asset.
