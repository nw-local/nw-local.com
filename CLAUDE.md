...
- **US spelling only, everywhere a reader can see it.** The audience is Washington State, so `favor` not `favour`, `color` not `colour`, `gray` not `grey`, `mold` not `mould`, `analyze` not `analyse`, `labeled` not `labelled`, `fiber` not `fibre`, `liter` not `litre`. This covers Sanity content, image asset `description` fields, Portable Text image `alt` and `caption`, table cells, and any user-facing string in the repo. Two things make it drift without anyone noticing. Scientific sources are frequently British or international, so text lifted or paraphrased from a paper arrives spelled the source's way: `Favoured temperature` reached a published table straight out of a botrytis paper. And **alt text is the blind spot** because nobody reads it while proofing: `pale grey backdrop` and `labelled as the trade` sat in two image `description` fields and rendered onto five pages. `make check-content-style` fails the build on the whole list.
